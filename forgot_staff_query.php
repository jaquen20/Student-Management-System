		  ...
		  if($total==1)
		   {
		   			...
		   			if ($confpass==$newpass) 
		   			{
		   				$Update_query="UPDATE `staff_tbl` SET `password`='$confpass' WHERE user_id='$uname'";
       					 ...
       					 header("location:stafflogin.php?status4= Successfull");

		   			}
		   			else
		   			{
		   				header("location:forget_staff.php?status= Confirm password Not Match");
		   			}
		   ...
		   {
		   		header("location:forget_staff.php?status2=uname&qustion");
		   }
